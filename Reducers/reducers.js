...
export function moviesReducer(state = [], action) {
    switch (action.type) {
        ...
        case  actions.RECEIVE_MOVIE:
            let existingIndex = state.findIndex((m) => m.id === action.movie.id);  //  в массиве стэйт для каждого элемента м если мы нашли элемент в стэйте с таким же айди как в параметре, то взять его индекс
